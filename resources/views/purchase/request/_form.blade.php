...
<div class="row">
  ...
  <div class="col-md-6">
    ...
    @endif
      <span class="help-block">data cabang tidak ada? <a class="text-red" href="{{ url('/master/branch/create') }}"
          target="_blank">buat cabang</a></span>
    </div>
    ...
    <div class="form-group @if($errors->has('destination')) has-error @endif">
      ...
      <option value="{{ $typeId }}" @if($destinationTypeId == $typeId) selected @endif>{{ $type['label'] }}</option>
    @endforeach
      </select>
    ...
    <table id="vue-dynamic-element" class="table table-bordered table-hover">
      ...
      <tbody>
        <tr v-for="(element, i) in elements" :key="element . id">
          ...
          <td>
            <input type="text" class="form-control" :name="`purchase_details[${i}][amount]`" :value="getAmount(i)"
              placeholder="estimation price">
          </td>
          <td>
