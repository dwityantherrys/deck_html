...
<div class="row">
  ...
  <div class="col-md-6">
    ...
    @endif
      <span class="help-block">data vendor tidak ada? <a class="text-red" href="{{ url('/master/vendor/create') }}"
    ...
    <table id="vue-dynamic-element" class="table table-bordered table-hover">
      ...
      <tbody>
        <tr v-for="(element, i) in elements" :key="element . id">
          ...
          <td>
            <input type="text" class="form-control" :name="`job_order_details[${i}][price]`" v-model="element.price"
              @change="isNumber(i, 'price')" placeholder="price">
          </td>
          <td>
            <input type="text" class="form-control" :name="`job_order_details[${i}][amount]`" :value="getAmount(i)"
              placeholder="amount">
          </td>
          <td>
