...
<div class="row">
  ...
  <div class="col-md-6">
    <div class="form-group @if($errors->has('date')) has-error @endif">
      ...
      <div class="input-group date">
        ...
        <input type="text" class="form-control pull-right" name="date"
          value="{{ empty($model->date) ? date('m/d/Y') : $model->date_formated }}" readonly>
      </div>
      @if($errors->has('date'))
    ...
    <div class="form-group @if($errors->has('job_order_id')) has-error @endif">
      ...
      <select class="has-ajax-form form-control" name="job_order_id" style="width: 100%;" tabindex="-1"
  ...
  <div class="col-md-6">
    ...
    @endif
      <span class="help-block">data vendor tidak ada? <a class="text-red" href="{{ url('master/customer/create') }}"
          target="_blank">buat vendor</a></span>
    </div>
    <div class="form-group @if($errors->has('remark')) has-error @endif">
        ...
        <tr v-for="(element, i) in elements" :key="element . id">
          <td>
            ...
            <vue-select2 :url="`{{ $baseBeApiUrl . '/items-service' }}`"
              :name="`delivery_note_details[${i}][item_material_id]`" :value="element . item_material_id"
          ...
          <td>
            <input type="text" class="form-control" :name="`delivery_note_details[${i}][quantity]`"
              v-model="element.quantity" @change="isNumber(i, 'quantity')" placeholder="quantity">
          </td>
          <td>
            <input type="text" class="form-control" :name="`delivery_note_details[${i}][price]`" v-model="element.price"
              @change="isNumber(i, 'price')" placeholder="estimation price">
          </td>
          <td>
            <input type="text" class="form-control" :name="`delivery_note_details[${i}][amount]`" :value="getAmount(i)"
