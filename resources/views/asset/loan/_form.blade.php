    ...
    <table id="vue-dynamic-element" class="table table-bordered table-hover">
      ...
      <tbody>
        <tr v-for="(element, i) in elements" :key="element.id">
          ...
          <td>
            <input type="text" class="form-control" :name="`loan_details[${i}][quantity]`" v-model="element.quantity"  @change="isNumber(i, 'quantity')" placeholder="quantity" readonly>
          </td>
          <td>
...
@section('js')
  ...
  Vue.component('vue-select2', {
    ...
    mounted: async function() {
      ...
      if(this.value) {
        await this.getDataById (this.value)
      }
    },
    destroyed: function () {
